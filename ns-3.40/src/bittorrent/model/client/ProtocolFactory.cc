...
{
  Ptr<PeerConnectorStrategyBase> peerConnectorStrategy = Create<PeerConnectorStrategyBase, Ptr<BitTorrentClient> > (std::move(client));
  strategyStore.push_back (peerConnectorStrategy);
  peerConnectorStrategy->DoInitialize ();

  Ptr<ChokeUnChokeStrategyBase> chokeUnChokeStrategy = Create<ChokeUnChokeStrategyBase, Ptr<BitTorrentClient> > (std::move(client));
  strategyStore.push_back (chokeUnChokeStrategy);
  chokeUnChokeStrategy->DoInitialize ();

  Ptr<PartSelectionStrategyBase> partSelectionStrategy = Create<PartSelectionStrategyBase, Ptr<BitTorrentClient> > (std::move(client));
  strategyStore.push_back (partSelectionStrategy);
  partSelectionStrategy->DoInitialize ();

  Ptr<RequestSchedulingStrategyBase> requestSchedulingStrategy = Create<RequestSchedulingStrategyBase, Ptr<BitTorrentClient > > (std::move(client));
...
void ProtocolFactory::CreateRarestFirstProtocol (Ptr<BitTorrentClient> client, std::list<Ptr<AbstractStrategy> > &strategyStore, Ptr<PeerConnectorStrategyBase>& aPeerConnectorStrategy)
{
  ...
  Ptr<RarestFirstPartSelectionStrategy> partSelectionStrategy = Create<RarestFirstPartSelectionStrategy, Ptr<BitTorrentClient> > (std::move(client));
  strategyStore.push_back (partSelectionStrategy);
  partSelectionStrategy->DoInitialize ();
...
//	partSelectionStrategy->DoInitialize();
  ...
  Ptr<BitTorrentVideoMetricsBase> videoMetricsBase = Create<BitTorrentVideoMetricsBase, Ptr<BitTorrentClient> > (std::move(client));
