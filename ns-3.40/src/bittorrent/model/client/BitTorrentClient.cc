...
void BitTorrentClient::StartApplication ()
{
  ...
  Ptr<ConstantRandomVariable> bitfieldFillerConst;
  uint32_t randomEndStart = bitfieldSize;
  switch (m_bitfieldFillType[0])
    {
    case 'e':
      bitfieldFillerConst = CreateObject<ConstantRandomVariable>();
      for (uint8_t i = 0; i < randomEndStart; ++i)
      {
         m_bitfield[i] = static_cast<uint8_t> (bitfieldFillerConst->GetInteger (0));
      }      
      break;
    case 'f':
      bitfieldFillerConst = CreateObject<ConstantRandomVariable>();   
      for (uint8_t i = 0; i < randomEndStart; ++i)
      {
         m_bitfield[i] = static_cast<uint8_t> (bitfieldFillerConst->GetInteger (255));
      }            
      break;
    case 'r':
    {
  ...
  Ptr<UniformRandomVariable> bitfieldFillerUni = CreateObject<UniformRandomVariable>();

  for (uint8_t i = randomEndStart; i < bitfieldSize; ++i)
    {
      m_bitfield[i] = static_cast<uint8_t> (bitfieldFillerUni->GetValue (0.0, 255.0));
    }
  // <-- Step 3b
